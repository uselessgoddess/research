...
impl fmt::Debug for Extensions {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(map) = &self.map {
            set.entries(map.values().map(|any_clone| TypeName(any_clone.as_ref().type_name())));
        }
        set.finish()
    }
}
...
fn test_extensions() {
    ...
    assert!(
        (dbg == "{shai::util::extensions::test_extensions::MyType, i32}")
            || (dbg == "{i32, shai::util::extensions::test_extensions::MyType}"),
        "{}",
        dbg
    );
